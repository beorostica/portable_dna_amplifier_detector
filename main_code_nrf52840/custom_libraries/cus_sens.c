...
static void on_write(cus_sens_t * p_cus, ble_evt_t const * p_ble_evt);
static void on_tx_complete(cus_sens_t * p_cus);
...
static uint32_t custom_value_char_add(cus_sens_t * p_cus, const cus_sens_init_t * p_cus_init)
{
    ...
    ble_uuid.uuid = CUSTOM_SENS_VALUE_CHAR_UUID;
...
static void on_connect(cus_sens_t * p_cus, ble_evt_t const * p_ble_evt)
{
    ...
    cus_sens_evt_t evt;
    evt.evt_type = CUS_SENS_EVT_CONNECTED;
...
static void on_write(cus_sens_t * p_cus, ble_evt_t const * p_ble_evt)
{
    ...
    if ((p_evt_write->handle == p_cus->custom_value_handles.cccd_handle) && (p_evt_write->len == 2))
    {
        ...
        if (p_cus->evt_handler != NULL)
        {
            ...
            {
                evt.evt_type = CUS_SENS_EVT_NOTIFICATION_ENABLED;
            }
            else
            {
                evt.evt_type = CUS_SENS_EVT_NOTIFICATION_DISABLED;
            }
...
uint32_t cus_sens_ble_init(cus_sens_t * p_cus, const cus_sens_init_t * p_cus_init)
{
    ...
    ble_uuid128_t base_uuid = {CUSTOM_SENS_SERVICE_UUID_BASE};
    err_code =  sd_ble_uuid_vs_add(&base_uuid, &p_cus->uuid_type);
    VERIFY_SUCCESS(err_code);
    
    ble_uuid.type = p_cus->uuid_type;
    ble_uuid.uuid = CUSTOM_SENS_SERVICE_UUID;
...
{
   cus_sens_t * p_cus = (cus_sens_t *) p_context;
